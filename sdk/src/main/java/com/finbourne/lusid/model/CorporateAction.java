...
public class CorporateAction {
  ...
  * @throws IOException if the JSON Element is invalid with respect to CorporateAction
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CorporateAction.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CorporateAction.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CorporateAction` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("transitions") != null && !jsonObj.get("transitions").isJsonNull()) {
        ...
        if (jsonArraytransitions != null) {
          ...
          for (int i = 0; i < jsonArraytransitions.size(); i++) {
            CorporateActionTransition.validateJsonElement(jsonArraytransitions.get(i));
          };
        }
      }
  }
