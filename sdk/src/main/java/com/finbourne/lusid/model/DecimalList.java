...
public class DecimalList extends ReferenceList {
  ...
  * @throws IOException if the JSON Element is invalid with respect to DecimalList
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DecimalList.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!DecimalList.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DecimalList` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
