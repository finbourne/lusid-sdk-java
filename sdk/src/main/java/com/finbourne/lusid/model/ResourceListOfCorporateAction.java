...
public class ResourceListOfCorporateAction {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResourceListOfCorporateAction
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceListOfCorporateAction.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResourceListOfCorporateAction is not found in the empty JSON string", ResourceListOfCorporateAction.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfCorporateAction.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfCorporateAction` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResourceListOfCorporateAction.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        CorporateAction.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResourceListOfCorporateAction.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResourceListOfCorporateAction' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResourceListOfCorporateAction> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResourceListOfCorporateAction.class));

       return (TypeAdapter<T>) new TypeAdapter<ResourceListOfCorporateAction>() {
           @Override
           public void write(JsonWriter out, ResourceListOfCorporateAction value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResourceListOfCorporateAction read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to ResourceListOfCorporateAction
  */
  public static ResourceListOfCorporateAction fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResourceListOfCorporateAction.class);
  }
