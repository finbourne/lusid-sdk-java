...
public class AggregatedReturn {
  ...
  * @throws IOException if the JSON Element is invalid with respect to AggregatedReturn
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AggregatedReturn.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!AggregatedReturn.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AggregatedReturn` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("compositeMembersWithoutReturn") != null && !jsonObj.get("compositeMembersWithoutReturn").isJsonNull()) {
        ...
        if (jsonArraycompositeMembersWithoutReturn != null) {
          ...
          for (int i = 0; i < jsonArraycompositeMembersWithoutReturn.size(); i++) {
            ResourceId.validateJsonElement(jsonArraycompositeMembersWithoutReturn.get(i));
          };
        }
      }
  }
