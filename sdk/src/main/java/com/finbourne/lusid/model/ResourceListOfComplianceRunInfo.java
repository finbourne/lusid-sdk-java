...
public class ResourceListOfComplianceRunInfo {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResourceListOfComplianceRunInfo
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceListOfComplianceRunInfo.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResourceListOfComplianceRunInfo is not found in the empty JSON string", ResourceListOfComplianceRunInfo.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfComplianceRunInfo.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfComplianceRunInfo` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResourceListOfComplianceRunInfo.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        ComplianceRunInfo.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResourceListOfComplianceRunInfo.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResourceListOfComplianceRunInfo' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResourceListOfComplianceRunInfo> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResourceListOfComplianceRunInfo.class));

       return (TypeAdapter<T>) new TypeAdapter<ResourceListOfComplianceRunInfo>() {
           @Override
           public void write(JsonWriter out, ResourceListOfComplianceRunInfo value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResourceListOfComplianceRunInfo read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to ResourceListOfComplianceRunInfo
  */
  public static ResourceListOfComplianceRunInfo fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResourceListOfComplianceRunInfo.class);
  }
