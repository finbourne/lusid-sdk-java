...
public class VendorModelRule {
  ...
  * @throws IOException if the JSON Element is invalid with respect to VendorModelRule
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!VendorModelRule.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!VendorModelRule.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `VendorModelRule` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("modelOptions") != null && !jsonObj.get("modelOptions").isJsonNull()) {
        ModelOptions.validateJsonElement(jsonObj.get("modelOptions"));
      }
