...
public class ResourceListOfAggregatedReturn {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResourceListOfAggregatedReturn
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceListOfAggregatedReturn.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResourceListOfAggregatedReturn is not found in the empty JSON string", ResourceListOfAggregatedReturn.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfAggregatedReturn.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfAggregatedReturn` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResourceListOfAggregatedReturn.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        AggregatedReturn.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResourceListOfAggregatedReturn.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResourceListOfAggregatedReturn' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResourceListOfAggregatedReturn> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResourceListOfAggregatedReturn.class));

       return (TypeAdapter<T>) new TypeAdapter<ResourceListOfAggregatedReturn>() {
           @Override
           public void write(JsonWriter out, ResourceListOfAggregatedReturn value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResourceListOfAggregatedReturn read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to ResourceListOfAggregatedReturn
  */
  public static ResourceListOfAggregatedReturn fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResourceListOfAggregatedReturn.class);
  }
