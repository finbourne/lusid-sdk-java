...
public class CustodianAccountsUpsertResponse {
  ...
  * @throws IOException if the JSON Element is invalid with respect to CustodianAccountsUpsertResponse
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CustodianAccountsUpsertResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CustodianAccountsUpsertResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CustodianAccountsUpsertResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("custodianAccounts") != null && !jsonObj.get("custodianAccounts").isJsonNull()) {
        ...
        if (jsonArraycustodianAccounts != null) {
          ...
          for (int i = 0; i < jsonArraycustodianAccounts.size(); i++) {
            CustodianAccount.validateJsonElement(jsonArraycustodianAccounts.get(i));
          };
        }
      }
