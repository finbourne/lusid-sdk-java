...
public class PagedResourceListOfAbor {
  ...
  * @throws IOException if the JSON Element is invalid with respect to PagedResourceListOfAbor
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PagedResourceListOfAbor.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PagedResourceListOfAbor is not found in the empty JSON string", PagedResourceListOfAbor.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!PagedResourceListOfAbor.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PagedResourceListOfAbor` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : PagedResourceListOfAbor.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        Abor.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PagedResourceListOfAbor.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PagedResourceListOfAbor' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PagedResourceListOfAbor> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PagedResourceListOfAbor.class));

       return (TypeAdapter<T>) new TypeAdapter<PagedResourceListOfAbor>() {
           @Override
           public void write(JsonWriter out, PagedResourceListOfAbor value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PagedResourceListOfAbor read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to PagedResourceListOfAbor
  */
  public static PagedResourceListOfAbor fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PagedResourceListOfAbor.class);
  }
