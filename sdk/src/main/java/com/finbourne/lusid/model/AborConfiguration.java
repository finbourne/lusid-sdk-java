...
public class AborConfiguration {
  ...
  * @throws IOException if the JSON Element is invalid with respect to AborConfiguration
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AborConfiguration.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!AborConfiguration.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AborConfiguration` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("postingModuleIds") != null && !jsonObj.get("postingModuleIds").isJsonNull()) {
        ...
        if (jsonArraypostingModuleIds != null) {
          ...
          for (int i = 0; i < jsonArraypostingModuleIds.size(); i++) {
            ResourceId.validateJsonElement(jsonArraypostingModuleIds.get(i));
          };
        }
      }
