...
public class OrderGraphBlockPlacementSynopsis {
  ...
  * @throws IOException if the JSON Element is invalid with respect to OrderGraphBlockPlacementSynopsis
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!OrderGraphBlockPlacementSynopsis.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!OrderGraphBlockPlacementSynopsis.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `OrderGraphBlockPlacementSynopsis` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArraydetails.size(); i++) {
        OrderGraphBlockPlacementDetail.validateJsonElement(jsonArraydetails.get(i));
      };
  }
