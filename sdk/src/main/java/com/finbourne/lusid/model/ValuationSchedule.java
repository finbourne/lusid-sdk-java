  ...
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ValuationSchedule.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ValuationSchedule` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      // ensure the optional json data is an array if present
      if (jsonObj.get("valuationDateTimes") != null && !jsonObj.get("valuationDateTimes").isJsonArray()) {
