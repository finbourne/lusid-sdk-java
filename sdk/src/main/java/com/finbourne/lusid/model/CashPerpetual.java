...
public class CashPerpetual extends LusidInstrument {
  ...
  * @throws IOException if the JSON Element is invalid with respect to CashPerpetual
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CashPerpetual.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CashPerpetual.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CashPerpetual` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
