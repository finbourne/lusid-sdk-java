...
public class HoldingAdjustmentWithDate {
  ...
  * @throws IOException if the JSON Element is invalid with respect to HoldingAdjustmentWithDate
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!HoldingAdjustmentWithDate.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in HoldingAdjustmentWithDate is not found in the empty JSON string", HoldingAdjustmentWithDate.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!HoldingAdjustmentWithDate.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `HoldingAdjustmentWithDate` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : HoldingAdjustmentWithDate.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!HoldingAdjustmentWithDate.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'HoldingAdjustmentWithDate' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<HoldingAdjustmentWithDate> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(HoldingAdjustmentWithDate.class));

       return (TypeAdapter<T>) new TypeAdapter<HoldingAdjustmentWithDate>() {
           @Override
           public void write(JsonWriter out, HoldingAdjustmentWithDate value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public HoldingAdjustmentWithDate read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to HoldingAdjustmentWithDate
  */
  public static HoldingAdjustmentWithDate fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, HoldingAdjustmentWithDate.class);
  }
