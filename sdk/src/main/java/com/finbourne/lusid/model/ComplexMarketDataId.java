...
public class ComplexMarketDataId {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ComplexMarketDataId
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ComplexMarketDataId.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ComplexMarketDataId.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ComplexMarketDataId` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
