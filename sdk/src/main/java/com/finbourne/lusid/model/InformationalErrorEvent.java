...
public class InformationalErrorEvent extends InstrumentEvent {
  ...
  * @throws IOException if the JSON Element is invalid with respect to InformationalErrorEvent
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!InformationalErrorEvent.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!InformationalErrorEvent.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `InformationalErrorEvent` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
