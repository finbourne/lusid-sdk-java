...
public class ResourceListOfPerformanceReturn {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResourceListOfPerformanceReturn
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceListOfPerformanceReturn.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfPerformanceReturn.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfPerformanceReturn` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        PerformanceReturn.validateJsonElement(jsonArrayvalues.get(i));
      };
