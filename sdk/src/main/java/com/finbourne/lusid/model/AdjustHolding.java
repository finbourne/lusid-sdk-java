...
public class AdjustHolding {
  ...
  * @throws IOException if the JSON Element is invalid with respect to AdjustHolding
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AdjustHolding.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!AdjustHolding.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AdjustHolding` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
