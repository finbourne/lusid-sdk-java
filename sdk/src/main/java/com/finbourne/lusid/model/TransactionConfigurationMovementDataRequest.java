...
public class TransactionConfigurationMovementDataRequest {
  ...
  * @throws IOException if the JSON Object is invalid with respect to TransactionConfigurationMovementDataRequest
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!TransactionConfigurationMovementDataRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!TransactionConfigurationMovementDataRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TransactionConfigurationMovementDataRequest` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if (jsonObj.get("mappings") != null && !jsonObj.get("mappings").isJsonNull()) {
        ...
        if (jsonArraymappings != null) {
          ...
          for (int i = 0; i < jsonArraymappings.size(); i++) {
            TransactionPropertyMappingRequest.validateJsonObject(jsonArraymappings.get(i).getAsJsonObject());
          };
        }
      }
