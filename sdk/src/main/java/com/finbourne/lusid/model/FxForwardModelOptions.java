...
public class FxForwardModelOptions extends ModelOptions {
  ...
  * @throws IOException if the JSON Object is invalid with respect to FxForwardModelOptions
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!FxForwardModelOptions.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!FxForwardModelOptions.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `FxForwardModelOptions` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
