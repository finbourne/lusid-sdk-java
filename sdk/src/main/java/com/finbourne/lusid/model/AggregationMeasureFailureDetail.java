...
public class AggregationMeasureFailureDetail {
  ...
  * @throws IOException if the JSON Element is invalid with respect to AggregationMeasureFailureDetail
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AggregationMeasureFailureDetail.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!AggregationMeasureFailureDetail.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AggregationMeasureFailureDetail` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
