...
public class ResourceListOfProcessedCommand {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResourceListOfProcessedCommand
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceListOfProcessedCommand.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResourceListOfProcessedCommand is not found in the empty JSON string", ResourceListOfProcessedCommand.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfProcessedCommand.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfProcessedCommand` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResourceListOfProcessedCommand.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        ProcessedCommand.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResourceListOfProcessedCommand.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResourceListOfProcessedCommand' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResourceListOfProcessedCommand> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResourceListOfProcessedCommand.class));

       return (TypeAdapter<T>) new TypeAdapter<ResourceListOfProcessedCommand>() {
           @Override
           public void write(JsonWriter out, ResourceListOfProcessedCommand value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResourceListOfProcessedCommand read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to ResourceListOfProcessedCommand
  */
  public static ResourceListOfProcessedCommand fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResourceListOfProcessedCommand.class);
  }
