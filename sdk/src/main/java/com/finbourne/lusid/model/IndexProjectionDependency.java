...
public class IndexProjectionDependency extends EconomicDependency {
  ...
  * @throws IOException if the JSON Element is invalid with respect to IndexProjectionDependency
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!IndexProjectionDependency.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!IndexProjectionDependency.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `IndexProjectionDependency` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
