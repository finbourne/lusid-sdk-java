...
public class ResourceListOfPortfolioTradeTicket {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResourceListOfPortfolioTradeTicket
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceListOfPortfolioTradeTicket.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResourceListOfPortfolioTradeTicket is not found in the empty JSON string", ResourceListOfPortfolioTradeTicket.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfPortfolioTradeTicket.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfPortfolioTradeTicket` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResourceListOfPortfolioTradeTicket.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        PortfolioTradeTicket.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResourceListOfPortfolioTradeTicket.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResourceListOfPortfolioTradeTicket' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResourceListOfPortfolioTradeTicket> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResourceListOfPortfolioTradeTicket.class));

       return (TypeAdapter<T>) new TypeAdapter<ResourceListOfPortfolioTradeTicket>() {
           @Override
           public void write(JsonWriter out, ResourceListOfPortfolioTradeTicket value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResourceListOfPortfolioTradeTicket read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to ResourceListOfPortfolioTradeTicket
  */
  public static ResourceListOfPortfolioTradeTicket fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResourceListOfPortfolioTradeTicket.class);
  }
