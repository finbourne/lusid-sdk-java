...
public class ScopeDefinition {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ScopeDefinition
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ScopeDefinition.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ScopeDefinition is not found in the empty JSON string", ScopeDefinition.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ScopeDefinition.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ScopeDefinition` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ScopeDefinition.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ScopeDefinition.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ScopeDefinition' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ScopeDefinition> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ScopeDefinition.class));

       return (TypeAdapter<T>) new TypeAdapter<ScopeDefinition>() {
           @Override
           public void write(JsonWriter out, ScopeDefinition value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ScopeDefinition read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of ScopeDefinition given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of ScopeDefinition
  * @throws IOException if the JSON string is invalid with respect to ScopeDefinition
  */
  public static ScopeDefinition fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ScopeDefinition.class);
  }

 /**
  * Convert an instance of ScopeDefinition to an JSON string
  *
  * @return JSON string
  */
