...
public class BucketedCashFlowResponse {
  ...
  * @throws IOException if the JSON Element is invalid with respect to BucketedCashFlowResponse
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BucketedCashFlowResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!BucketedCashFlowResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BucketedCashFlowResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
