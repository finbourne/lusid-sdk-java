...
public class JELinesQueryParameters {
  ...
  * @throws IOException if the JSON Element is invalid with respect to JELinesQueryParameters
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!JELinesQueryParameters.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!JELinesQueryParameters.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `JELinesQueryParameters` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
