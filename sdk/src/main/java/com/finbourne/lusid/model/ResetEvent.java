...
public class ResetEvent extends InstrumentEvent {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResetEvent
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResetEvent.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResetEvent.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResetEvent` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
