...
public class ExecutionSetRequest {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ExecutionSetRequest
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ExecutionSetRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ExecutionSetRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ExecutionSetRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("requests") != null && !jsonObj.get("requests").isJsonNull()) {
        ...
        if (jsonArrayrequests != null) {
          ...
          for (int i = 0; i < jsonArrayrequests.size(); i++) {
            ExecutionRequest.validateJsonElement(jsonArrayrequests.get(i));
          };
        }
      }
  }
