...
public class ResourceListOfRelationship {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResourceListOfRelationship
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceListOfRelationship.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResourceListOfRelationship is not found in the empty JSON string", ResourceListOfRelationship.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfRelationship.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfRelationship` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResourceListOfRelationship.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        Relationship.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResourceListOfRelationship.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResourceListOfRelationship' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResourceListOfRelationship> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResourceListOfRelationship.class));

       return (TypeAdapter<T>) new TypeAdapter<ResourceListOfRelationship>() {
           @Override
           public void write(JsonWriter out, ResourceListOfRelationship value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResourceListOfRelationship read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of ResourceListOfRelationship given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of ResourceListOfRelationship
  * @throws IOException if the JSON string is invalid with respect to ResourceListOfRelationship
  */
  public static ResourceListOfRelationship fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResourceListOfRelationship.class);
  }

 /**
  * Convert an instance of ResourceListOfRelationship to an JSON string
  *
  * @return JSON string
  */
