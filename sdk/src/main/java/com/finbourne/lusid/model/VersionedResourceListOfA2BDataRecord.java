...
public class VersionedResourceListOfA2BDataRecord {
  ...
  * @throws IOException if the JSON Object is invalid with respect to VersionedResourceListOfA2BDataRecord
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!VersionedResourceListOfA2BDataRecord.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!VersionedResourceListOfA2BDataRecord.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `VersionedResourceListOfA2BDataRecord` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        A2BDataRecord.validateJsonObject(jsonArrayvalues.get(i).getAsJsonObject());
      };
