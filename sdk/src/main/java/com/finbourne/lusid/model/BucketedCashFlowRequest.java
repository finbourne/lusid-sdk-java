...
public class BucketedCashFlowRequest {
  ...
  * @throws IOException if the JSON Element is invalid with respect to BucketedCashFlowRequest
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BucketedCashFlowRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!BucketedCashFlowRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BucketedCashFlowRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
