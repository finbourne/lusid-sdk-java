...
public class FxRateSchedule extends Schedule {
  ...
  * @throws IOException if the JSON Element is invalid with respect to FxRateSchedule
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!FxRateSchedule.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!FxRateSchedule.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `FxRateSchedule` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
