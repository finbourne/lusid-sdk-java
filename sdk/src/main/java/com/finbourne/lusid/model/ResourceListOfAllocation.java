...
public class ResourceListOfAllocation {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResourceListOfAllocation
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceListOfAllocation.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResourceListOfAllocation is not found in the empty JSON string", ResourceListOfAllocation.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfAllocation.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfAllocation` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResourceListOfAllocation.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResourceListOfAllocation.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResourceListOfAllocation' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResourceListOfAllocation> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResourceListOfAllocation.class));

       return (TypeAdapter<T>) new TypeAdapter<ResourceListOfAllocation>() {
           @Override
           public void write(JsonWriter out, ResourceListOfAllocation value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResourceListOfAllocation read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to ResourceListOfAllocation
  */
  public static ResourceListOfAllocation fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResourceListOfAllocation.class);
  }
