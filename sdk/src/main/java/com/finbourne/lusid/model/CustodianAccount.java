...
public class CustodianAccount {
  ...
  * @throws IOException if the JSON Element is invalid with respect to CustodianAccount
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CustodianAccount.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CustodianAccount.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CustodianAccount` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
