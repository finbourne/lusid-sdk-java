...
public class NextValueInSequenceResponse {
  ...
  * @throws IOException if the JSON Element is invalid with respect to NextValueInSequenceResponse
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!NextValueInSequenceResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in NextValueInSequenceResponse is not found in the empty JSON string", NextValueInSequenceResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!NextValueInSequenceResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `NextValueInSequenceResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : NextValueInSequenceResponse.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!NextValueInSequenceResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'NextValueInSequenceResponse' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<NextValueInSequenceResponse> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(NextValueInSequenceResponse.class));

       return (TypeAdapter<T>) new TypeAdapter<NextValueInSequenceResponse>() {
           @Override
           public void write(JsonWriter out, NextValueInSequenceResponse value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public NextValueInSequenceResponse read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to NextValueInSequenceResponse
  */
  public static NextValueInSequenceResponse fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, NextValueInSequenceResponse.class);
  }
