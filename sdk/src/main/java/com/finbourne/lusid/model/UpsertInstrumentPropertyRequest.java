...
public class UpsertInstrumentPropertyRequest {
  ...
  * @throws IOException if the JSON Element is invalid with respect to UpsertInstrumentPropertyRequest
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UpsertInstrumentPropertyRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!UpsertInstrumentPropertyRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UpsertInstrumentPropertyRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
