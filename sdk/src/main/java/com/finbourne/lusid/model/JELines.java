...
public class JELines {
  ...
  * @throws IOException if the JSON Element is invalid with respect to JELines
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!JELines.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!JELines.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `JELines` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("postingModuleId") != null && !jsonObj.get("postingModuleId").isJsonNull()) {
        ResourceId.validateJsonElement(jsonObj.get("postingModuleId"));
      }
