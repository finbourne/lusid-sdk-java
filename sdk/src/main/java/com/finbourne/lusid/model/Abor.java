  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!Abor.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Abor` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("aborConfig") != null && !jsonObj.get("aborConfig").isJsonNull()) {
        ResourceId.validateJsonElement(jsonObj.get("aborConfig"));
      }
