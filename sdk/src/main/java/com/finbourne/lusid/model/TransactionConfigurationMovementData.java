...
public class TransactionConfigurationMovementData {
  ...
  * @throws IOException if the JSON Object is invalid with respect to TransactionConfigurationMovementData
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!TransactionConfigurationMovementData.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!TransactionConfigurationMovementData.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TransactionConfigurationMovementData` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if (jsonObj.get("mappings") != null && !jsonObj.get("mappings").isJsonNull()) {
        ...
        if (jsonArraymappings != null) {
          ...
          for (int i = 0; i < jsonArraymappings.size(); i++) {
            TransactionPropertyMapping.validateJsonObject(jsonArraymappings.get(i).getAsJsonObject());
          };
        }
      }
