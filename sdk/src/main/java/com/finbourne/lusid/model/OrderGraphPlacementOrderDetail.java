...
public class OrderGraphPlacementOrderDetail {
  ...
  * @throws IOException if the JSON Element is invalid with respect to OrderGraphPlacementOrderDetail
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!OrderGraphPlacementOrderDetail.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!OrderGraphPlacementOrderDetail.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `OrderGraphPlacementOrderDetail` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
