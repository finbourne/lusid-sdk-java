...
public class InterestRateSwap extends LusidInstrument {
  ...
  * @throws IOException if the JSON Object is invalid with respect to InterestRateSwap
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!InterestRateSwap.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!InterestRateSwap.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `InterestRateSwap` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
