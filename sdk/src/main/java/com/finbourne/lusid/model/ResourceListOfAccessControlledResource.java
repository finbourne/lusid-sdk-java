...
public class ResourceListOfAccessControlledResource {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ResourceListOfAccessControlledResource
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ResourceListOfAccessControlledResource.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfAccessControlledResource.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfAccessControlledResource` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        AccessControlledResource.validateJsonObject(jsonArrayvalues.get(i).getAsJsonObject());
      };
