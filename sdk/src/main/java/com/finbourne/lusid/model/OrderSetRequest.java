...
public class OrderSetRequest {
  ...
  **/
  ...
  static {
    ...
    openapiFields.add("orderRequests");
  ...
  * @throws IOException if the JSON Element is invalid with respect to OrderSetRequest
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!OrderSetRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in OrderSetRequest is not found in the empty JSON string", OrderSetRequest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!OrderSetRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `OrderSetRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      if (jsonObj.get("orderRequests") != null && !jsonObj.get("orderRequests").isJsonNull()) {
        JsonArray jsonArrayorderRequests = jsonObj.getAsJsonArray("orderRequests");
        if (jsonArrayorderRequests != null) {
          // ensure the json data is an array
          if (!jsonObj.get("orderRequests").isJsonArray()) {
            throw new IllegalArgumentException(String.format("Expected the field `orderRequests` to be an array in the JSON string but got `%s`", jsonObj.get("orderRequests").toString()));
          }

          // validate the optional field `orderRequests` (array)
          for (int i = 0; i < jsonArrayorderRequests.size(); i++) {
            OrderRequest.validateJsonElement(jsonArrayorderRequests.get(i));
          };
        }
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!OrderSetRequest.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'OrderSetRequest' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<OrderSetRequest> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(OrderSetRequest.class));

       return (TypeAdapter<T>) new TypeAdapter<OrderSetRequest>() {
           @Override
           public void write(JsonWriter out, OrderSetRequest value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public OrderSetRequest read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to OrderSetRequest
  */
  public static OrderSetRequest fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, OrderSetRequest.class);
  }
