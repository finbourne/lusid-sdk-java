...
public class IsBusinessDayResponse {
  ...
  * @throws IOException if the JSON Element is invalid with respect to IsBusinessDayResponse
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!IsBusinessDayResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!IsBusinessDayResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `IsBusinessDayResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
