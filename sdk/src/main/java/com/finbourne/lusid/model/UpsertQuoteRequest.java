...
public class UpsertQuoteRequest {
  ...
  * @throws IOException if the JSON Object is invalid with respect to UpsertQuoteRequest
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!UpsertQuoteRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!UpsertQuoteRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UpsertQuoteRequest` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
