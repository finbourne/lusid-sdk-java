...
public class Equity extends LusidInstrument {
  ...
  * @throws IOException if the JSON Element is invalid with respect to Equity
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Equity.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!Equity.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Equity` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
