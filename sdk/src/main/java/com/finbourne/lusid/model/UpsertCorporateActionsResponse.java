...
public class UpsertCorporateActionsResponse {
  ...
  * @throws IOException if the JSON Element is invalid with respect to UpsertCorporateActionsResponse
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UpsertCorporateActionsResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!UpsertCorporateActionsResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UpsertCorporateActionsResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
