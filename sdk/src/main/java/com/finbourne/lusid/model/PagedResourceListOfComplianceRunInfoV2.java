...
public class PagedResourceListOfComplianceRunInfoV2 {
  ...
  * @throws IOException if the JSON Element is invalid with respect to PagedResourceListOfComplianceRunInfoV2
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PagedResourceListOfComplianceRunInfoV2.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!PagedResourceListOfComplianceRunInfoV2.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PagedResourceListOfComplianceRunInfoV2` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        ComplianceRunInfoV2.validateJsonElement(jsonArrayvalues.get(i));
      };
