...
public class AggregatedReturnsDispersionRequest {
  ...
  * @throws IOException if the JSON Element is invalid with respect to AggregatedReturnsDispersionRequest
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AggregatedReturnsDispersionRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!AggregatedReturnsDispersionRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AggregatedReturnsDispersionRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
