...
public class ResourceListOfPortfolioCashLadder {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResourceListOfPortfolioCashLadder
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceListOfPortfolioCashLadder.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResourceListOfPortfolioCashLadder is not found in the empty JSON string", ResourceListOfPortfolioCashLadder.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfPortfolioCashLadder.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfPortfolioCashLadder` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResourceListOfPortfolioCashLadder.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        PortfolioCashLadder.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResourceListOfPortfolioCashLadder.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResourceListOfPortfolioCashLadder' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResourceListOfPortfolioCashLadder> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResourceListOfPortfolioCashLadder.class));

       return (TypeAdapter<T>) new TypeAdapter<ResourceListOfPortfolioCashLadder>() {
           @Override
           public void write(JsonWriter out, ResourceListOfPortfolioCashLadder value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResourceListOfPortfolioCashLadder read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to ResourceListOfPortfolioCashLadder
  */
  public static ResourceListOfPortfolioCashLadder fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResourceListOfPortfolioCashLadder.class);
  }
