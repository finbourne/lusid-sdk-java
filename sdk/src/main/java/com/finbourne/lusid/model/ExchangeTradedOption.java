...
public class ExchangeTradedOption extends LusidInstrument {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ExchangeTradedOption
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ExchangeTradedOption.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ExchangeTradedOption.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ExchangeTradedOption` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
