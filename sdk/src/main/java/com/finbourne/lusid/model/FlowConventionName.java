...
public class FlowConventionName {
  ...
  * @throws IOException if the JSON Element is invalid with respect to FlowConventionName
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!FlowConventionName.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in FlowConventionName is not found in the empty JSON string", FlowConventionName.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!FlowConventionName.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `FlowConventionName` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : FlowConventionName.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!FlowConventionName.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'FlowConventionName' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<FlowConventionName> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(FlowConventionName.class));

       return (TypeAdapter<T>) new TypeAdapter<FlowConventionName>() {
           @Override
           public void write(JsonWriter out, FlowConventionName value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public FlowConventionName read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to FlowConventionName
  */
  public static FlowConventionName fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, FlowConventionName.class);
  }
