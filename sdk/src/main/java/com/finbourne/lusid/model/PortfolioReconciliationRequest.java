...
public class PortfolioReconciliationRequest {
  ...
  * @throws IOException if the JSON Element is invalid with respect to PortfolioReconciliationRequest
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PortfolioReconciliationRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!PortfolioReconciliationRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PortfolioReconciliationRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
