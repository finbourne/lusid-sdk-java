...
public class ReconciliationRunId {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ReconciliationRunId
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ReconciliationRunId.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ReconciliationRunId.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ReconciliationRunId` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("reconciliation") != null && !jsonObj.get("reconciliation").isJsonNull()) {
        ReconciliationId.validateJsonElement(jsonObj.get("reconciliation"));
      }
