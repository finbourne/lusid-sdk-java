...
public class ExpandedGroup {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ExpandedGroup
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ExpandedGroup.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ExpandedGroup.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ExpandedGroup` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("values") != null && !jsonObj.get("values").isJsonNull()) {
        ...
        if (jsonArrayvalues != null) {
          ...
          for (int i = 0; i < jsonArrayvalues.size(); i++) {
            CompletePortfolio.validateJsonElement(jsonArrayvalues.get(i));
          };
        }
      }
      if (jsonObj.get("subGroups") != null && !jsonObj.get("subGroups").isJsonNull()) {
        ...
        if (jsonArraysubGroups != null) {
          ...
          for (int i = 0; i < jsonArraysubGroups.size(); i++) {
            ExpandedGroup.validateJsonElement(jsonArraysubGroups.get(i));
          };
        }
      }
