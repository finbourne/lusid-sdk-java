...
public class TransactionPropertyMapping {
  ...
  * @throws IOException if the JSON Element is invalid with respect to TransactionPropertyMapping
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TransactionPropertyMapping.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in TransactionPropertyMapping is not found in the empty JSON string", TransactionPropertyMapping.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!TransactionPropertyMapping.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TransactionPropertyMapping` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : TransactionPropertyMapping.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!TransactionPropertyMapping.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'TransactionPropertyMapping' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<TransactionPropertyMapping> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(TransactionPropertyMapping.class));

       return (TypeAdapter<T>) new TypeAdapter<TransactionPropertyMapping>() {
           @Override
           public void write(JsonWriter out, TransactionPropertyMapping value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public TransactionPropertyMapping read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to TransactionPropertyMapping
  */
  public static TransactionPropertyMapping fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, TransactionPropertyMapping.class);
  }
