...
public class PropertyValueIn extends MatchCriterion {
  ...
  * @throws IOException if the JSON Element is invalid with respect to PropertyValueIn
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PropertyValueIn.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!PropertyValueIn.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PropertyValueIn` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
