...
public class PagedResourceListOfCustodianAccount {
  ...
  * @throws IOException if the JSON Element is invalid with respect to PagedResourceListOfCustodianAccount
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PagedResourceListOfCustodianAccount.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PagedResourceListOfCustodianAccount is not found in the empty JSON string", PagedResourceListOfCustodianAccount.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!PagedResourceListOfCustodianAccount.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PagedResourceListOfCustodianAccount` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : PagedResourceListOfCustodianAccount.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        CustodianAccount.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PagedResourceListOfCustodianAccount.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PagedResourceListOfCustodianAccount' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PagedResourceListOfCustodianAccount> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PagedResourceListOfCustodianAccount.class));

       return (TypeAdapter<T>) new TypeAdapter<PagedResourceListOfCustodianAccount>() {
           @Override
           public void write(JsonWriter out, PagedResourceListOfCustodianAccount value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PagedResourceListOfCustodianAccount read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to PagedResourceListOfCustodianAccount
  */
  public static PagedResourceListOfCustodianAccount fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PagedResourceListOfCustodianAccount.class);
  }
