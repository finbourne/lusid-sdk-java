...
public class PagedResourceListOfOrderGraphBlock {
  ...
  * @throws IOException if the JSON Element is invalid with respect to PagedResourceListOfOrderGraphBlock
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PagedResourceListOfOrderGraphBlock.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PagedResourceListOfOrderGraphBlock is not found in the empty JSON string", PagedResourceListOfOrderGraphBlock.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!PagedResourceListOfOrderGraphBlock.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PagedResourceListOfOrderGraphBlock` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : PagedResourceListOfOrderGraphBlock.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        OrderGraphBlock.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PagedResourceListOfOrderGraphBlock.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PagedResourceListOfOrderGraphBlock' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PagedResourceListOfOrderGraphBlock> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PagedResourceListOfOrderGraphBlock.class));

       return (TypeAdapter<T>) new TypeAdapter<PagedResourceListOfOrderGraphBlock>() {
           @Override
           public void write(JsonWriter out, PagedResourceListOfOrderGraphBlock value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PagedResourceListOfOrderGraphBlock read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to PagedResourceListOfOrderGraphBlock
  */
  public static PagedResourceListOfOrderGraphBlock fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PagedResourceListOfOrderGraphBlock.class);
  }
