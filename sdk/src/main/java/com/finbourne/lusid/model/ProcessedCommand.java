...
public class ProcessedCommand {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ProcessedCommand
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ProcessedCommand.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ProcessedCommand.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ProcessedCommand` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      User.validateJsonElement(jsonObj.get("userId"));
  }
