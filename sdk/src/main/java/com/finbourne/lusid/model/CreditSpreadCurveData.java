...
public class CreditSpreadCurveData extends ComplexMarketData {
  ...
  * @throws IOException if the JSON Object is invalid with respect to CreditSpreadCurveData
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!CreditSpreadCurveData.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CreditSpreadCurveData.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CreditSpreadCurveData` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
