...
public class ResourceListOfPortfolioCashFlow {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResourceListOfPortfolioCashFlow
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceListOfPortfolioCashFlow.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResourceListOfPortfolioCashFlow is not found in the empty JSON string", ResourceListOfPortfolioCashFlow.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfPortfolioCashFlow.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfPortfolioCashFlow` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResourceListOfPortfolioCashFlow.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        PortfolioCashFlow.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResourceListOfPortfolioCashFlow.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResourceListOfPortfolioCashFlow' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResourceListOfPortfolioCashFlow> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResourceListOfPortfolioCashFlow.class));

       return (TypeAdapter<T>) new TypeAdapter<ResourceListOfPortfolioCashFlow>() {
           @Override
           public void write(JsonWriter out, ResourceListOfPortfolioCashFlow value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResourceListOfPortfolioCashFlow read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to ResourceListOfPortfolioCashFlow
  */
  public static ResourceListOfPortfolioCashFlow fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResourceListOfPortfolioCashFlow.class);
  }
