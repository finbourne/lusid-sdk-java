...
public class ResourceListOfDataType {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResourceListOfDataType
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceListOfDataType.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResourceListOfDataType is not found in the empty JSON string", ResourceListOfDataType.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfDataType.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfDataType` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResourceListOfDataType.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        DataType.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResourceListOfDataType.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResourceListOfDataType' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResourceListOfDataType> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResourceListOfDataType.class));

       return (TypeAdapter<T>) new TypeAdapter<ResourceListOfDataType>() {
           @Override
           public void write(JsonWriter out, ResourceListOfDataType value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResourceListOfDataType read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of ResourceListOfDataType given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of ResourceListOfDataType
  * @throws IOException if the JSON string is invalid with respect to ResourceListOfDataType
  */
  public static ResourceListOfDataType fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResourceListOfDataType.class);
  }

 /**
  * Convert an instance of ResourceListOfDataType to an JSON string
  *
  * @return JSON string
  */
