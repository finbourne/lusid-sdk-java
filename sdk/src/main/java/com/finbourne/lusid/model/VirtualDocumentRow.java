...
public class VirtualDocumentRow {
  ...
  * @throws IOException if the JSON Element is invalid with respect to VirtualDocumentRow
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!VirtualDocumentRow.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!VirtualDocumentRow.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `VirtualDocumentRow` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the optional field `rowData`
      if (jsonObj.get("rowData") != null && !jsonObj.get("rowData").isJsonNull()) {
        GroupedResultOfAddressKey.validateJsonElement(jsonObj.get("rowData"));
      }
  }
