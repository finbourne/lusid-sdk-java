...
public class ResourceListOfOrderInstruction {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResourceListOfOrderInstruction
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceListOfOrderInstruction.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResourceListOfOrderInstruction is not found in the empty JSON string", ResourceListOfOrderInstruction.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfOrderInstruction.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfOrderInstruction` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResourceListOfOrderInstruction.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResourceListOfOrderInstruction.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResourceListOfOrderInstruction' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResourceListOfOrderInstruction> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResourceListOfOrderInstruction.class));

       return (TypeAdapter<T>) new TypeAdapter<ResourceListOfOrderInstruction>() {
           @Override
           public void write(JsonWriter out, ResourceListOfOrderInstruction value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResourceListOfOrderInstruction read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to ResourceListOfOrderInstruction
  */
  public static ResourceListOfOrderInstruction fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResourceListOfOrderInstruction.class);
  }
