...
public class PagedResourceListOfOrderInstruction {
  ...
  * @throws IOException if the JSON Element is invalid with respect to PagedResourceListOfOrderInstruction
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PagedResourceListOfOrderInstruction.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PagedResourceListOfOrderInstruction is not found in the empty JSON string", PagedResourceListOfOrderInstruction.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!PagedResourceListOfOrderInstruction.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PagedResourceListOfOrderInstruction` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : PagedResourceListOfOrderInstruction.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        OrderInstruction.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PagedResourceListOfOrderInstruction.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PagedResourceListOfOrderInstruction' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PagedResourceListOfOrderInstruction> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PagedResourceListOfOrderInstruction.class));

       return (TypeAdapter<T>) new TypeAdapter<PagedResourceListOfOrderInstruction>() {
           @Override
           public void write(JsonWriter out, PagedResourceListOfOrderInstruction value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PagedResourceListOfOrderInstruction read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to PagedResourceListOfOrderInstruction
  */
  public static PagedResourceListOfOrderInstruction fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PagedResourceListOfOrderInstruction.class);
  }
