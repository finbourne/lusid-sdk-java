...
public class ReconciliationLeftRightAddressKeyPair {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ReconciliationLeftRightAddressKeyPair
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ReconciliationLeftRightAddressKeyPair.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ReconciliationLeftRightAddressKeyPair.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ReconciliationLeftRightAddressKeyPair` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
