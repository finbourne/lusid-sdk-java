...
public class OrderGraphPlacementPlacementSynopsis {
  ...
  * @throws IOException if the JSON Element is invalid with respect to OrderGraphPlacementPlacementSynopsis
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!OrderGraphPlacementPlacementSynopsis.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!OrderGraphPlacementPlacementSynopsis.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `OrderGraphPlacementPlacementSynopsis` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArraydetails.size(); i++) {
        OrderGraphPlacementChildPlacementDetail.validateJsonElement(jsonArraydetails.get(i));
      };
  }
