...
public class ResourceListOfCalendarDate {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResourceListOfCalendarDate
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceListOfCalendarDate.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResourceListOfCalendarDate is not found in the empty JSON string", ResourceListOfCalendarDate.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfCalendarDate.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfCalendarDate` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResourceListOfCalendarDate.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        CalendarDate.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResourceListOfCalendarDate.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResourceListOfCalendarDate' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResourceListOfCalendarDate> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResourceListOfCalendarDate.class));

       return (TypeAdapter<T>) new TypeAdapter<ResourceListOfCalendarDate>() {
           @Override
           public void write(JsonWriter out, ResourceListOfCalendarDate value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResourceListOfCalendarDate read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to ResourceListOfCalendarDate
  */
  public static ResourceListOfCalendarDate fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResourceListOfCalendarDate.class);
  }
