...
public class UpsertInstrumentsResponse {
  ...
  * @throws IOException if the JSON Element is invalid with respect to UpsertInstrumentsResponse
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UpsertInstrumentsResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in UpsertInstrumentsResponse is not found in the empty JSON string", UpsertInstrumentsResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!UpsertInstrumentsResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UpsertInstrumentsResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!UpsertInstrumentsResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'UpsertInstrumentsResponse' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<UpsertInstrumentsResponse> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(UpsertInstrumentsResponse.class));

       return (TypeAdapter<T>) new TypeAdapter<UpsertInstrumentsResponse>() {
           @Override
           public void write(JsonWriter out, UpsertInstrumentsResponse value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public UpsertInstrumentsResponse read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to UpsertInstrumentsResponse
  */
  public static UpsertInstrumentsResponse fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, UpsertInstrumentsResponse.class);
  }
