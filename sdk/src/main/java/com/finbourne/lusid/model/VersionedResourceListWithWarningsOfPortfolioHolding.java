...
public class VersionedResourceListWithWarningsOfPortfolioHolding {
  ...
  * @throws IOException if the JSON Element is invalid with respect to VersionedResourceListWithWarningsOfPortfolioHolding
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!VersionedResourceListWithWarningsOfPortfolioHolding.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!VersionedResourceListWithWarningsOfPortfolioHolding.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `VersionedResourceListWithWarningsOfPortfolioHolding` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        PortfolioHolding.validateJsonElement(jsonArrayvalues.get(i));
      };
