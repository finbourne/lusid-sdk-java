...
public class UpsertReferencePortfolioConstituentsRequest {
  ...
  * @throws IOException if the JSON Element is invalid with respect to UpsertReferencePortfolioConstituentsRequest
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UpsertReferencePortfolioConstituentsRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!UpsertReferencePortfolioConstituentsRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UpsertReferencePortfolioConstituentsRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayconstituents.size(); i++) {
        ReferencePortfolioConstituentRequest.validateJsonElement(jsonArrayconstituents.get(i));
      };
  }
