...
public class PagedResourceListOfAllocation {
  ...
  * @throws IOException if the JSON Element is invalid with respect to PagedResourceListOfAllocation
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PagedResourceListOfAllocation.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PagedResourceListOfAllocation is not found in the empty JSON string", PagedResourceListOfAllocation.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!PagedResourceListOfAllocation.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PagedResourceListOfAllocation` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : PagedResourceListOfAllocation.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        Allocation.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PagedResourceListOfAllocation.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PagedResourceListOfAllocation' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PagedResourceListOfAllocation> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PagedResourceListOfAllocation.class));

       return (TypeAdapter<T>) new TypeAdapter<PagedResourceListOfAllocation>() {
           @Override
           public void write(JsonWriter out, PagedResourceListOfAllocation value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PagedResourceListOfAllocation read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of PagedResourceListOfAllocation given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of PagedResourceListOfAllocation
  * @throws IOException if the JSON string is invalid with respect to PagedResourceListOfAllocation
  */
  public static PagedResourceListOfAllocation fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PagedResourceListOfAllocation.class);
  }

 /**
  * Convert an instance of PagedResourceListOfAllocation to an JSON string
  *
  * @return JSON string
  */
