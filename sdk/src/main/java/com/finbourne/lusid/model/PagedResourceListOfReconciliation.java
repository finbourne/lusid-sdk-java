...
public class PagedResourceListOfReconciliation {
  ...
  * @throws IOException if the JSON Element is invalid with respect to PagedResourceListOfReconciliation
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PagedResourceListOfReconciliation.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PagedResourceListOfReconciliation is not found in the empty JSON string", PagedResourceListOfReconciliation.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!PagedResourceListOfReconciliation.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PagedResourceListOfReconciliation` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : PagedResourceListOfReconciliation.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        Reconciliation.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PagedResourceListOfReconciliation.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PagedResourceListOfReconciliation' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PagedResourceListOfReconciliation> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PagedResourceListOfReconciliation.class));

       return (TypeAdapter<T>) new TypeAdapter<PagedResourceListOfReconciliation>() {
           @Override
           public void write(JsonWriter out, PagedResourceListOfReconciliation value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PagedResourceListOfReconciliation read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to PagedResourceListOfReconciliation
  */
  public static PagedResourceListOfReconciliation fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PagedResourceListOfReconciliation.class);
  }
