...
public class CutLabelDefinition {
  ...
  * @throws IOException if the JSON Element is invalid with respect to CutLabelDefinition
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CutLabelDefinition.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in CutLabelDefinition is not found in the empty JSON string", CutLabelDefinition.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CutLabelDefinition.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CutLabelDefinition` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      // validate the optional field `cutLocalTime`
      if (jsonObj.get("cutLocalTime") != null && !jsonObj.get("cutLocalTime").isJsonNull()) {
        CutLocalTime.validateJsonElement(jsonObj.get("cutLocalTime"));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!CutLabelDefinition.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'CutLabelDefinition' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<CutLabelDefinition> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(CutLabelDefinition.class));

       return (TypeAdapter<T>) new TypeAdapter<CutLabelDefinition>() {
           @Override
           public void write(JsonWriter out, CutLabelDefinition value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public CutLabelDefinition read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to CutLabelDefinition
  */
  public static CutLabelDefinition fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, CutLabelDefinition.class);
  }
