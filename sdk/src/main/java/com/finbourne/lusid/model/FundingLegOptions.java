...
public class FundingLegOptions extends ModelOptions {
  ...
  * @throws IOException if the JSON Object is invalid with respect to FundingLegOptions
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!FundingLegOptions.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!FundingLegOptions.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `FundingLegOptions` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
