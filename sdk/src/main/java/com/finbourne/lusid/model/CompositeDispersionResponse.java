...
public class CompositeDispersionResponse {
  ...
  * @throws IOException if the JSON Element is invalid with respect to CompositeDispersionResponse
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CompositeDispersionResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in CompositeDispersionResponse is not found in the empty JSON string", CompositeDispersionResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CompositeDispersionResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CompositeDispersionResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!CompositeDispersionResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'CompositeDispersionResponse' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<CompositeDispersionResponse> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(CompositeDispersionResponse.class));

       return (TypeAdapter<T>) new TypeAdapter<CompositeDispersionResponse>() {
           @Override
           public void write(JsonWriter out, CompositeDispersionResponse value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public CompositeDispersionResponse read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to CompositeDispersionResponse
  */
  public static CompositeDispersionResponse fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, CompositeDispersionResponse.class);
  }
