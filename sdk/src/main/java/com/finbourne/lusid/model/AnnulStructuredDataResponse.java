...
public class AnnulStructuredDataResponse {
  ...
  * @throws IOException if the JSON Element is invalid with respect to AnnulStructuredDataResponse
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AnnulStructuredDataResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!AnnulStructuredDataResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `AnnulStructuredDataResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
