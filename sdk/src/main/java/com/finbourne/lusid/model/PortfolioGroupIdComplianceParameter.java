...
public class PortfolioGroupIdComplianceParameter extends ComplianceParameter {
  ...
  * @throws IOException if the JSON Element is invalid with respect to PortfolioGroupIdComplianceParameter
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PortfolioGroupIdComplianceParameter.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!PortfolioGroupIdComplianceParameter.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PortfolioGroupIdComplianceParameter` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
