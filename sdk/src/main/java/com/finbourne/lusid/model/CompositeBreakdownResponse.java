...
public class CompositeBreakdownResponse {
  ...
  * @throws IOException if the JSON Element is invalid with respect to CompositeBreakdownResponse
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CompositeBreakdownResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in CompositeBreakdownResponse is not found in the empty JSON string", CompositeBreakdownResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CompositeBreakdownResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CompositeBreakdownResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : CompositeBreakdownResponse.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!CompositeBreakdownResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'CompositeBreakdownResponse' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<CompositeBreakdownResponse> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(CompositeBreakdownResponse.class));

       return (TypeAdapter<T>) new TypeAdapter<CompositeBreakdownResponse>() {
           @Override
           public void write(JsonWriter out, CompositeBreakdownResponse value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public CompositeBreakdownResponse read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to CompositeBreakdownResponse
  */
  public static CompositeBreakdownResponse fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, CompositeBreakdownResponse.class);
  }
