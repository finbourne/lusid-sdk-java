...
public class ResultDataKeyRule extends ResultKeyRule {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ResultDataKeyRule
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ResultDataKeyRule.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResultDataKeyRule.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResultDataKeyRule` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
