...
public class OpaqueMarketData extends ComplexMarketData {
  ...
  * @throws IOException if the JSON Element is invalid with respect to OpaqueMarketData
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!OpaqueMarketData.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!OpaqueMarketData.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `OpaqueMarketData` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
