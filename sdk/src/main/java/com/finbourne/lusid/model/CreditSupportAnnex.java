...
public class CreditSupportAnnex {
  ...
  * @throws IOException if the JSON Element is invalid with respect to CreditSupportAnnex
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CreditSupportAnnex.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CreditSupportAnnex.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CreditSupportAnnex` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
