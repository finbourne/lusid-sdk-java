...
public class ResultValue0D extends ResultValue {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResultValue0D
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResultValue0D.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResultValue0D is not found in the empty JSON string", ResultValue0D.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResultValue0D.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResultValue0D` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResultValue0D.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResultValue0D.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResultValue0D' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResultValue0D> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResultValue0D.class));

       return (TypeAdapter<T>) new TypeAdapter<ResultValue0D>() {
           @Override
           public void write(JsonWriter out, ResultValue0D value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResultValue0D read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to ResultValue0D
  */
  public static ResultValue0D fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResultValue0D.class);
  }
