...
public class FundingLegModelOptions extends ModelOptions {
  ...
  * @throws IOException if the JSON Element is invalid with respect to FundingLegModelOptions
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!FundingLegModelOptions.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in FundingLegModelOptions is not found in the empty JSON string", FundingLegModelOptions.openapiRequiredFields.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : FundingLegModelOptions.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!FundingLegModelOptions.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'FundingLegModelOptions' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<FundingLegModelOptions> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(FundingLegModelOptions.class));

       return (TypeAdapter<T>) new TypeAdapter<FundingLegModelOptions>() {
           @Override
           public void write(JsonWriter out, FundingLegModelOptions value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public FundingLegModelOptions read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to FundingLegModelOptions
  */
  public static FundingLegModelOptions fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, FundingLegModelOptions.class);
  }
