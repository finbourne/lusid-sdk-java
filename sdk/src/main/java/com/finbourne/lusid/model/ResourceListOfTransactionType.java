...
public class ResourceListOfTransactionType {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResourceListOfTransactionType
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceListOfTransactionType.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResourceListOfTransactionType is not found in the empty JSON string", ResourceListOfTransactionType.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfTransactionType.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfTransactionType` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResourceListOfTransactionType.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        TransactionType.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResourceListOfTransactionType.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResourceListOfTransactionType' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResourceListOfTransactionType> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResourceListOfTransactionType.class));

       return (TypeAdapter<T>) new TypeAdapter<ResourceListOfTransactionType>() {
           @Override
           public void write(JsonWriter out, ResourceListOfTransactionType value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResourceListOfTransactionType read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to ResourceListOfTransactionType
  */
  public static ResourceListOfTransactionType fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResourceListOfTransactionType.class);
  }
