...
public class CompositeBreakdown {
  ...
  * @throws IOException if the JSON Object is invalid with respect to CompositeBreakdown
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!CompositeBreakdown.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CompositeBreakdown.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CompositeBreakdown` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if (jsonObj.get("composite") != null && !jsonObj.get("composite").isJsonNull()) {
        PortfolioReturnBreakdown.validateJsonObject(jsonObj.getAsJsonObject("composite"));
      }
      if (jsonObj.get("constituents") != null && !jsonObj.get("constituents").isJsonNull()) {
        ...
        if (jsonArrayconstituents != null) {
          ...
          for (int i = 0; i < jsonArrayconstituents.size(); i++) {
            PortfolioReturnBreakdown.validateJsonObject(jsonArrayconstituents.get(i).getAsJsonObject());
          };
        }
      }
  }
