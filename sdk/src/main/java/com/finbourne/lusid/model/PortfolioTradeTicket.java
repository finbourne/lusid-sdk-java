...
public class PortfolioTradeTicket {
  ...
  * @throws IOException if the JSON Element is invalid with respect to PortfolioTradeTicket
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PortfolioTradeTicket.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!PortfolioTradeTicket.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PortfolioTradeTicket` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("tradeTicket") != null && !jsonObj.get("tradeTicket").isJsonNull()) {
        LusidTradeTicket.validateJsonElement(jsonObj.get("tradeTicket"));
      }
  }
