...
public class UpsertResultValuesDataRequest {
  ...
  * @throws IOException if the JSON Element is invalid with respect to UpsertResultValuesDataRequest
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UpsertResultValuesDataRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!UpsertResultValuesDataRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UpsertResultValuesDataRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("resultValue") != null && !jsonObj.get("resultValue").isJsonNull()) {
        ResultValue.validateJsonElement(jsonObj.get("resultValue"));
      }
  }
