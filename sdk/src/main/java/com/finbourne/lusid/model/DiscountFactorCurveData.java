...
public class DiscountFactorCurveData extends ComplexMarketData {
  ...
  * @throws IOException if the JSON Element is invalid with respect to DiscountFactorCurveData
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DiscountFactorCurveData.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!DiscountFactorCurveData.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DiscountFactorCurveData` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
