...
public class LifeCycleEventValue extends ResultValue {
  ...
  * @throws IOException if the JSON Element is invalid with respect to LifeCycleEventValue
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!LifeCycleEventValue.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!LifeCycleEventValue.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `LifeCycleEventValue` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
