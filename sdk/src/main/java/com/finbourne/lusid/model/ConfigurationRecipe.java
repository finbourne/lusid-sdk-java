...
public class ConfigurationRecipe {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ConfigurationRecipe
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ConfigurationRecipe.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ConfigurationRecipe.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ConfigurationRecipe` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if (jsonObj.get("market") != null && !jsonObj.get("market").isJsonNull()) {
        MarketContext.validateJsonObject(jsonObj.getAsJsonObject("market"));
      }
      // validate the optional field `pricing`
      if (jsonObj.get("pricing") != null && !jsonObj.get("pricing").isJsonNull()) {
        PricingContext.validateJsonObject(jsonObj.getAsJsonObject("pricing"));
      }
      // validate the optional field `aggregation`
      if (jsonObj.get("aggregation") != null && !jsonObj.get("aggregation").isJsonNull()) {
        AggregationContext.validateJsonObject(jsonObj.getAsJsonObject("aggregation"));
      }
      if (jsonObj.get("inheritedRecipes") != null && !jsonObj.get("inheritedRecipes").isJsonNull()) {
        ...
        if (jsonArrayinheritedRecipes != null) {
          ...
          for (int i = 0; i < jsonArrayinheritedRecipes.size(); i++) {
            ResourceId.validateJsonObject(jsonArrayinheritedRecipes.get(i).getAsJsonObject());
          };
        }
      }
      ...
      if (jsonObj.get("holding") != null && !jsonObj.get("holding").isJsonNull()) {
        HoldingContext.validateJsonObject(jsonObj.getAsJsonObject("holding"));
      }
  }
