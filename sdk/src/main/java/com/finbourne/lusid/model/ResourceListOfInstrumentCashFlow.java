...
public class ResourceListOfInstrumentCashFlow {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResourceListOfInstrumentCashFlow
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceListOfInstrumentCashFlow.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResourceListOfInstrumentCashFlow is not found in the empty JSON string", ResourceListOfInstrumentCashFlow.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfInstrumentCashFlow.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfInstrumentCashFlow` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResourceListOfInstrumentCashFlow.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        InstrumentCashFlow.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResourceListOfInstrumentCashFlow.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResourceListOfInstrumentCashFlow' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResourceListOfInstrumentCashFlow> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResourceListOfInstrumentCashFlow.class));

       return (TypeAdapter<T>) new TypeAdapter<ResourceListOfInstrumentCashFlow>() {
           @Override
           public void write(JsonWriter out, ResourceListOfInstrumentCashFlow value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResourceListOfInstrumentCashFlow read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to ResourceListOfInstrumentCashFlow
  */
  public static ResourceListOfInstrumentCashFlow fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResourceListOfInstrumentCashFlow.class);
  }
