...
public class CapFloor extends LusidInstrument {
  ...
  * @throws IOException if the JSON Object is invalid with respect to CapFloor
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!CapFloor.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CapFloor.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CapFloor` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
