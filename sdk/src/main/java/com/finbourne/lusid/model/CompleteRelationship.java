...
public class CompleteRelationship {
  ...
  * @throws IOException if the JSON Element is invalid with respect to CompleteRelationship
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CompleteRelationship.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CompleteRelationship.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CompleteRelationship` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      RelatedEntity.validateJsonElement(jsonObj.get("sourceEntity"));
      // validate the required field `targetEntity`
      RelatedEntity.validateJsonElement(jsonObj.get("targetEntity"));
