...
public class ResultValueDateTimeOffset extends ResultValue {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResultValueDateTimeOffset
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResultValueDateTimeOffset.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResultValueDateTimeOffset.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResultValueDateTimeOffset` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
