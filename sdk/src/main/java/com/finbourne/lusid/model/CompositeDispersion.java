...
public class CompositeDispersion {
  ...
  * @throws IOException if the JSON Element is invalid with respect to CompositeDispersion
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CompositeDispersion.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CompositeDispersion.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CompositeDispersion` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("constituentsInScope") != null && !jsonObj.get("constituentsInScope").isJsonNull()) {
        ...
        if (jsonArrayconstituentsInScope != null) {
          ...
          for (int i = 0; i < jsonArrayconstituentsInScope.size(); i++) {
            ResourceId.validateJsonElement(jsonArrayconstituentsInScope.get(i));
          };
        }
      }
      if (jsonObj.get("constituentsExcluded") != null && !jsonObj.get("constituentsExcluded").isJsonNull()) {
        ...
        if (jsonArrayconstituentsExcluded != null) {
          ...
          for (int i = 0; i < jsonArrayconstituentsExcluded.size(); i++) {
            ResourceId.validateJsonElement(jsonArrayconstituentsExcluded.get(i));
          };
        }
      }
  }
