...
public class A2BBreakdown {
  ...
  * @throws IOException if the JSON Element is invalid with respect to A2BBreakdown
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!A2BBreakdown.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!A2BBreakdown.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `A2BBreakdown` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
