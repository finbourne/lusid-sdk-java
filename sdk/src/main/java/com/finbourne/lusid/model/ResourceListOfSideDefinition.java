...
public class ResourceListOfSideDefinition {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResourceListOfSideDefinition
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceListOfSideDefinition.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResourceListOfSideDefinition is not found in the empty JSON string", ResourceListOfSideDefinition.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfSideDefinition.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfSideDefinition` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResourceListOfSideDefinition.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        SideDefinition.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResourceListOfSideDefinition.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResourceListOfSideDefinition' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResourceListOfSideDefinition> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResourceListOfSideDefinition.class));

       return (TypeAdapter<T>) new TypeAdapter<ResourceListOfSideDefinition>() {
           @Override
           public void write(JsonWriter out, ResourceListOfSideDefinition value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResourceListOfSideDefinition read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to ResourceListOfSideDefinition
  */
  public static ResourceListOfSideDefinition fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResourceListOfSideDefinition.class);
  }
