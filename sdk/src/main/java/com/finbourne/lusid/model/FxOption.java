...
public class FxOption extends LusidInstrument {
  ...
  * @throws IOException if the JSON Element is invalid with respect to FxOption
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!FxOption.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!FxOption.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `FxOption` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
