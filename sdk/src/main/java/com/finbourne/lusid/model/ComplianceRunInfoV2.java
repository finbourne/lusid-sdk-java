...
public class ComplianceRunInfoV2 {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ComplianceRunInfoV2
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ComplianceRunInfoV2.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ComplianceRunInfoV2.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ComplianceRunInfoV2` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
