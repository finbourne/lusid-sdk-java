...
public class EquitySwap extends LusidInstrument {
  ...
  * @throws IOException if the JSON Element is invalid with respect to EquitySwap
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!EquitySwap.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!EquitySwap.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `EquitySwap` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
