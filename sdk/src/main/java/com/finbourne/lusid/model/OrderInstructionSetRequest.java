...
public class OrderInstructionSetRequest {
  ...
  * @throws IOException if the JSON Element is invalid with respect to OrderInstructionSetRequest
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!OrderInstructionSetRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!OrderInstructionSetRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `OrderInstructionSetRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("requests") != null && !jsonObj.get("requests").isJsonNull()) {
        ...
        if (jsonArrayrequests != null) {
          ...
          for (int i = 0; i < jsonArrayrequests.size(); i++) {
            OrderInstructionRequest.validateJsonElement(jsonArrayrequests.get(i));
          };
        }
      }
  }
