...
public class TrialBalanceQueryParameters {
  ...
  * @throws IOException if the JSON Object is invalid with respect to TrialBalanceQueryParameters
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!TrialBalanceQueryParameters.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!TrialBalanceQueryParameters.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TrialBalanceQueryParameters` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
