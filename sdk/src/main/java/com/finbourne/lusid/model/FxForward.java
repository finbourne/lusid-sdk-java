...
public class FxForward extends LusidInstrument {
  ...
  * @throws IOException if the JSON Object is invalid with respect to FxForward
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!FxForward.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!FxForward.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `FxForward` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
