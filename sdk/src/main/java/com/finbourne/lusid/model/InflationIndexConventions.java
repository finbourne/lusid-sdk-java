...
public class InflationIndexConventions {
  ...
  * @throws IOException if the JSON Object is invalid with respect to InflationIndexConventions
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!InflationIndexConventions.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!InflationIndexConventions.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `InflationIndexConventions` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
