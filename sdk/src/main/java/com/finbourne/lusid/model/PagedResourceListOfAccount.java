...
public class PagedResourceListOfAccount {
  ...
  * @throws IOException if the JSON Element is invalid with respect to PagedResourceListOfAccount
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PagedResourceListOfAccount.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PagedResourceListOfAccount is not found in the empty JSON string", PagedResourceListOfAccount.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!PagedResourceListOfAccount.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PagedResourceListOfAccount` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : PagedResourceListOfAccount.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        Account.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PagedResourceListOfAccount.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PagedResourceListOfAccount' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PagedResourceListOfAccount> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PagedResourceListOfAccount.class));

       return (TypeAdapter<T>) new TypeAdapter<PagedResourceListOfAccount>() {
           @Override
           public void write(JsonWriter out, PagedResourceListOfAccount value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PagedResourceListOfAccount read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to PagedResourceListOfAccount
  */
  public static PagedResourceListOfAccount fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PagedResourceListOfAccount.class);
  }
