...
public class PropertyKeyComplianceParameter extends ComplianceParameter {
  ...
  * @throws IOException if the JSON Element is invalid with respect to PropertyKeyComplianceParameter
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PropertyKeyComplianceParameter.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in PropertyKeyComplianceParameter is not found in the empty JSON string", PropertyKeyComplianceParameter.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!PropertyKeyComplianceParameter.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PropertyKeyComplianceParameter` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : PropertyKeyComplianceParameter.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!PropertyKeyComplianceParameter.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'PropertyKeyComplianceParameter' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<PropertyKeyComplianceParameter> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(PropertyKeyComplianceParameter.class));

       return (TypeAdapter<T>) new TypeAdapter<PropertyKeyComplianceParameter>() {
           @Override
           public void write(JsonWriter out, PropertyKeyComplianceParameter value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public PropertyKeyComplianceParameter read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to PropertyKeyComplianceParameter
  */
  public static PropertyKeyComplianceParameter fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, PropertyKeyComplianceParameter.class);
  }
