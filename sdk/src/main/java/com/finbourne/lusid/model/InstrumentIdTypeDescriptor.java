...
public class InstrumentIdTypeDescriptor {
  ...
  * @throws IOException if the JSON Element is invalid with respect to InstrumentIdTypeDescriptor
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!InstrumentIdTypeDescriptor.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in InstrumentIdTypeDescriptor is not found in the empty JSON string", InstrumentIdTypeDescriptor.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!InstrumentIdTypeDescriptor.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `InstrumentIdTypeDescriptor` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : InstrumentIdTypeDescriptor.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!InstrumentIdTypeDescriptor.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'InstrumentIdTypeDescriptor' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<InstrumentIdTypeDescriptor> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(InstrumentIdTypeDescriptor.class));

       return (TypeAdapter<T>) new TypeAdapter<InstrumentIdTypeDescriptor>() {
           @Override
           public void write(JsonWriter out, InstrumentIdTypeDescriptor value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public InstrumentIdTypeDescriptor read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to InstrumentIdTypeDescriptor
  */
  public static InstrumentIdTypeDescriptor fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, InstrumentIdTypeDescriptor.class);
  }
