...
public class ResourceListOfPortfolio {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResourceListOfPortfolio
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResourceListOfPortfolio.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResourceListOfPortfolio is not found in the empty JSON string", ResourceListOfPortfolio.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResourceListOfPortfolio.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResourceListOfPortfolio` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResourceListOfPortfolio.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayvalues.size(); i++) {
        Portfolio.validateJsonElement(jsonArrayvalues.get(i));
      };
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResourceListOfPortfolio.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResourceListOfPortfolio' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResourceListOfPortfolio> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResourceListOfPortfolio.class));

       return (TypeAdapter<T>) new TypeAdapter<ResourceListOfPortfolio>() {
           @Override
           public void write(JsonWriter out, ResourceListOfPortfolio value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResourceListOfPortfolio read(JsonReader in) throws IOException {
  ...
  * @throws IOException if the JSON string is invalid with respect to ResourceListOfPortfolio
  */
  public static ResourceListOfPortfolio fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResourceListOfPortfolio.class);
  }
