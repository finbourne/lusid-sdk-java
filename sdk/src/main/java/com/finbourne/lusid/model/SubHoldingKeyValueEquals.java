...
public class SubHoldingKeyValueEquals extends MatchCriterion {
  ...
  * @throws IOException if the JSON Element is invalid with respect to SubHoldingKeyValueEquals
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SubHoldingKeyValueEquals.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!SubHoldingKeyValueEquals.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SubHoldingKeyValueEquals` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
