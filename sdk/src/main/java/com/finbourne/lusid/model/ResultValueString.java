...
public class ResultValueString extends ResultValue {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResultValueString
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResultValueString.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ResultValueString.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResultValueString` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
